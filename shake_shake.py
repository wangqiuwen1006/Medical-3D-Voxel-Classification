# -*- coding: utf-8 -*-
"""
Created on Mon Jun 15 10:27:03 2020
...
class ShakeShake(torch.autograd.Function):
    ...
    def forward(ctx, x1, x2, training=True):
        if training:
            ...
            alpha = alpha.view(alpha.size(0), 1, 1, 1, 1).expand_as(x1)
        else:
            alpha = 0.5
    ...
    def backward(ctx, grad_output):
        ...
        beta = beta.view(beta.size(0), 1, 1, 1, 1).expand_as(grad_output)
        beta = Variable(beta)
...
class Shortcut(nn.Module):
    ...
    def forward(self, x):
        ...
        h1 = F.avg_pool3d(h, 1, self.stride)
        h1 = self.conv1(h1)

        h2 = F.avg_pool3d(F.pad(h, (-1, 1, -1, 1)), 1, self.stride)
        h2 = self.conv2(h2)
...
class ShakeResNet(nn.Module):
    ...
    def __init__(self, depth, base_width, num_classes):
        ...
        self.conv_1 = nn.Conv2d(1, in_chs[0], 3, padding=1)
